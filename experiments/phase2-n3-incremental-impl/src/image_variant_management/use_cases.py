...
SATISFIES shared.ports.CopyRenderSpecPort natively via get_copy_render_spec,
which projects an ImageCopy into the neutral CopyRenderSpec DTO (enums -> .value
str, crops -> plain tuples; R-08 is NOT applied here, both crops are exposed).
...
class ImageVariantManagementUseCases:
    ...
    # =====================================================================
    # Consumer read port satisfaction (C-CONSUMER-PORTS): this IS
    # CopyRenderSpecPort. Native projection ImageCopy -> neutral CopyRenderSpec.
    # Enums become str via .value; crops become plain tuples. R-08 is NOT
    # applied here (both crops exposed); RENDERING UC-02 applies it.
    ...
    def get_copy_render_spec(self, copy_id: uuid.UUID) -> CopyRenderSpec | None:
        ...
        manual = (
            (copy.manual_crop.x, copy.manual_crop.y,
             copy.manual_crop.width, copy.manual_crop.height)
