...
| 入力 | `docs/capability-bom-audit/samples/` の 7 ファイルのみ |
...
| 項目 | 内容 |
| --- | --- |
| 場所 | docs/capability-bom-audit/samples/README.md L4 |
...
GRID_COMPOSITION は `docs/capability-bom-audit/samples/` 直下、
IMAGE_VARIANT_MANAGEMENT は `docs/capability-bom-audit/samples/image-variant-management/` 配下。
...
v0.3 で `docs/capability-bom-audit/samples/grid-composition/` への移行を行う想定。
...
| 入力 | `docs/capability-bom-audit/samples/image-variant-management/` 6 ファイル + 境界参照 (GRID 20/21) |
...
これは方法論本体への **新規昇格候補 (G-1)** であり、`docs/capability-bom-audit/methodology/`
...
| --- | --- |
| 実施日 | 2026-05-29 |
| 実装 | `docs/capability-bom-audit/tools/bom-conformance-check/checker.py` (執筆者自作の照合ツール、PyYAML) |
| 対象 BOM | GRID / IMAGE_VARIANT / RENDERING の 21-*.yaml |
| 対象実装 | `experiments/phase2-cocompose-impl/` (F-1/F-2/D-3 が実在する n=2 実装) |
| 方法論側規範 | `../methodology/22-bom-conformance-check.md` |
...
- `python docs/capability-bom-audit/tools/bom-conformance-check/checker.py` を **修正前後で 2 回実行**し、
...
2. n=4 以降 (Coordinator パターンの実体化) / 実プロジェクトでの試験運用
3. 方法論本体 (リポジトリ methodology/ 01〜10) への 11〜14 / 21 / 22 の昇格
...
| **昇格ポリシー** (`methodology/README`) | baseline 固定までは本体へ昇格しない (再昇格の手戻り回避) という draft→promoted 規範 |
...
3. baseline を前提に方法論本体 (リポジトリ methodology/ 01〜10) へ 11〜14 / 21 / 22 を昇格
