...
| **決定的検査器** (後段) | [`../bom-conformance-check/checker.py --authoring <bom.yaml>`](../bom-conformance-check/) | 単一 BOM: 構造ルール (SCHEMA/C3/PRECOND/REF/UI) + AI タグの機械的 enforcement (PROV)。`--authoring-set` で複数 BOM 横断 (XREF/XSYM/XSHARED) |
