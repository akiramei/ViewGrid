...
`docs/capability-bom-audit/tools/bom-conformance-check/checker.py` を GRID/IMGVAR/RENDERING の BOM と
...
- **対象の指定 (重要)**: ゲートは **生成物の src を CLI で明示** して回す:
  ...
  BOM は常に正準の `docs/capability-bom-audit/samples/` を参照する。
...
- **coverage manifest**: **(UC, failure_reason) ペア単位** で `guaranteed_by` / dynamically-probed /
  ...
  **未検証 (動的 probe 未整備) を可視化**し「pass=全部 OK」の誤読を防ぐ。現状の動的 probe は focused
  ...
  未検証項目は `../evaluation/91-findings-ledger.md` で追跡する。
...
- 実証根拠: `docs/capability-bom-audit/evaluation/90-feasibility-notes.md` Addendum I
- 照合ツール: `docs/capability-bom-audit/tools/bom-conformance-check/checker.py`
- 契約: `docs/capability-bom-audit/samples/00-convention-contract.md` (C-VALUE-SEMANTICS / C-IDENTITY-BOUNDARY)
